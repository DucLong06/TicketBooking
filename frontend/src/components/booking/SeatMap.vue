<template>
	<div
		class="theater-container bg-gray-900 text-white rounded-lg overflow-hidden shadow-2xl"
	>
		<!-- Legend & Controls -->
		<div
			class="legend-bar bg-gray-800 px-4 py-3 border-b border-gray-700 sticky top-0 z-10"
		>
			<div class="flex justify-between items-center mb-3">
				...
				<div class="text-sm text-gray-400">Cuộn để xem tất cả ghế</div>
			</div>

			<div class="flex flex-wrap justify-center gap-4 text-xs">
				<div class="flex items-center gap-2">
					<div
						class="w-6 h-6 bg-yellow-500 rounded-full border border-yellow-400"
					></div>
					<span>Đang trống</span>
				</div>
				<div class="flex items-center gap-2">
					<div
						class="w-6 h-6 bg-green-500 rounded-full border border-green-400 flex items-center justify-center"
					>
				...
				<div class="flex items-center gap-2">
					<div
						class="w-6 h-6 bg-red-800 rounded-full border border-red-700 flex items-center justify-center"
					>
				...
				<div class="flex items-center gap-2">
					<div
						class="w-6 h-6 bg-yellow-200 rounded-full border border-yellow-400"
					></div>
					<span>Đang giữ</span>
				</div>
			</div>
			...
			>
				...
				<div class="stage-area py-6 mb-8">
					<div class="flex justify-center">
						<div
							class="stage bg-gradient-to-b from-gray-300 to-gray-400 text-gray-800 px-20 py-4 rounded-full shadow-lg"
						>
				...
				<div class="rows-container space-y-2 max-w-none mx-auto">
					...
					<div class="seat-row flex items-center justify-center">
						<div
							class="row-label w-6 text-center text-sm font-bold text-gray-400 flex-shrink-0"
						>
							A
						</div>
						<div
							class="seats-container flex items-center justify-center gap-1"
						>
							<div
								v-for="n in 20"
								:key="`A-${n}`"
					...
					>
						...
						>
							...
							<div
								...
								:key="`${row}-${
									n +
									getLeftSectionCount(row) +
									getMiddleSectionCount(row)
								}`"
								:class="[
									'seat w-6 h-6 transition-all duration-200 cursor-pointer flex items-center justify-center font-bold text-xs rounded-full border',
									getSeatColorClass(
					...
					<div
						class="balcony-section flex items-center justify-center gap-8 my-4"
					>
						...
						<div class="center-balcony">
							...
							>
								<div
									class="row-label w-6 text-center text-sm font-bold text-gray-400"
								>
									R
								</div>
								<div
						...
						<div
							class="loge-right flex flex-col items-center gap-2"
						>
							<div class="text-sm font-bold text-purple-400">
								LG
							</div>
					...
					<!-- Floor 2 Separator -->
					<div
						class="floor-separator border-t-2 border-dashed border-gray-600 mt-8 pt-6"
					>
						<div class="text-center mb-6">
					...
					>
						...
						>
							<div
								...
								@mouseenter="
									showTooltip(rowData.label, n, $event)
								"
								@mouseleave="hideTooltip"
							>
...
const priceCategories = computed(() => {
	return seatMapData.value?.price_categories || {};
});
...
const loadSeatMap = async () => {
	...
	loading.value = true;
	try {
		const response = await bookingAPI.getSeatMap(props.performanceId);
		seatMapData.value = response.data;

		// Update seat statuses from API data
		const statusMap = new Map();
		response.data.seats.forEach((seat) => {
			statusMap.set(seat.id, seat.status);
		});
		seatStatuses.value = statusMap;
	} catch (error) {
